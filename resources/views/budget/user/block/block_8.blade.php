    ...
    <tbody>
        @foreach ($info['budget'] as $value) 
            ...
            <tr>
                ...
                <td><b>{{ number_format($sum_cb, 2, ',', ' ') }}</b></td>
                <td><font color="blue"><b>{{ number_format($sum_fu, 2, ',', ' ') }}</b></td>
            </tr>
            @else
            <tr>
                <td style="height: 65px;" class="sticky-col"><p class="text-scale">{{ $value['ekr']['title'] }}</p></td>
                <td>{{ $value['ekr']['ekr'] }}</td>
                <td>{{ number_format($sum_cb, 2, ',', ' ') }}</td>
                <td><font color="blue">{{ number_format($sum_fu, 2, ',', ' ') }}</td>
            </tr>
            @endif
        @endforeach
        @php
